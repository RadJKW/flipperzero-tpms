...
#define M_OPL_TPMSReceiverMenuItemArray_t()                                    \
  ARRAY_OPLIST(TPMSReceiverMenuItemArray, M_POD_OPLIST)
...
void tpms_view_receiver_set_rssi(TPMSReceiver *instance, float rssi) {
  ...
  with_view_model(
      instance->view, TPMSReceiverModel * model,
      {
...
void tpms_view_receiver_set_lock(TPMSReceiver *tpms_receiver, TPMSLock lock) {
  ...
  if (lock == TPMSLockOn) {
    ...
    with_view_model(
        tpms_receiver->view, TPMSReceiverModel * model,
        { model->bar_show = TPMSReceiverBarShowLock; }, true);
    furi_timer_start(tpms_receiver->lock_timer, furi_ms_to_ticks(1000));
  } else {
    with_view_model(
        tpms_receiver->view, TPMSReceiverModel * model,
        { model->bar_show = TPMSReceiverBarShowDefault; }, true);
  }
}

void tpms_view_receiver_set_callback(TPMSReceiver *tpms_receiver,
                                     TPMSReceiverCallback callback,
                                     void *context) {
  furi_assert(tpms_receiver);
  ...
  with_view_model(
      tpms_receiver->view, TPMSReceiverModel * model,
      {
        ...
        if (history_item > 3 && model->idx >= (int16_t)(history_item - 1)) {
          ...
          model->list_offset = CLAMP(model->list_offset + 1,
                                     (int16_t)(history_item - bounds), 0);
        } else if (model->list_offset > model->idx - bounds) {
          model->list_offset =
              CLAMP(model->idx - 1, (int16_t)(history_item - bounds), 0);
        }
      },
      true);
}

void tpms_view_receiver_add_item_to_menu(TPMSReceiver *tpms_receiver,
...
void tpms_view_receiver_add_data_statusbar(TPMSReceiver *tpms_receiver,
...
static void tpms_view_receiver_draw_frame(Canvas *canvas, uint16_t idx,
                                          bool scrollbar) {
  canvas_set_color(canvas, ColorBlack);
  canvas_draw_box(canvas, 0, 0 + idx * FRAME_HEIGHT, scrollbar ? 122 : 127,
                  FRAME_HEIGHT);
...
void tpms_view_receiver_draw(Canvas *canvas, TPMSReceiverModel *model) {
  ...
  for (size_t i = 0; i < MIN(model->history_item, MENU_ITEMS); ++i) {
    ...
    canvas_draw_str(canvas, 4, 9 + i * FRAME_HEIGHT,
  ...
  if (model->history_item == 0) {
    canvas_draw_icon(canvas, 0, 0,
                     model->external_radio ? &I_Fishing_123x52
                                           : &I_Scanning_123x52);
  ...
  if (model->relearn_active) {
    canvas_draw_str(canvas, 2, 62,
                    furi_string_get_cstr(model->relearn_status_str));
  }
...
static void tpms_view_receiver_lock_timer_callback(void *context) {
  ...
  if (tpms_receiver->lock_count < UNLOCK_CNT) {
    tpms_receiver->callback(TPMSCustomEventViewReceiverOffDisplay,
                            tpms_receiver->context);
  } else {
    tpms_receiver->lock = TPMSLockOff;
    tpms_receiver->callback(TPMSCustomEventViewReceiverUnlock,
                            tpms_receiver->context);
  }
  tpms_receiver->lock_count = 0;
}
...
static void tpms_relearn_stop(void *context) {
  ...
  if (tpms_receiver->relearn_active) {
    ...
    with_view_model(
        tpms_receiver->view, TPMSReceiverModel * model,
        { model->relearn_active = false; }, true);
  }
}
...
static void tpms_relearn_burst_timer_callback(void *context) {
  ...
  if (tpms_receiver->relearn_burst_phase_on) {
    ...
    furi_timer_start(tpms_receiver->relearn_burst_timer,
                     furi_ms_to_ticks(pattern->off_ms));
  } else {
    furi_hal_rfid_tim_read_start(
        125000, tpms_relearn_duty_to_float(tpms_receiver->relearn_duty));
    tpms_receiver->relearn_burst_phase_on = true;
    furi_timer_start(tpms_receiver->relearn_burst_timer,
                     furi_ms_to_ticks(pattern->on_ms));
  }
}
...
static void tpms_relearn_start(TPMSReceiver *tpms_receiver) {
  ...
  uint32_t runtime_ms =
      tpms_relearn_runtime_to_ms(tpms_receiver->relearn_runtime);
  if (runtime_ms != 0) {
    furi_timer_start(tpms_receiver->relearn_runtime_timer,
                     furi_ms_to_ticks(runtime_ms));
  }
  ...
  with_view_model(
      tpms_receiver->view, TPMSReceiverModel * model,
      {
        ...
        furi_string_printf(model->relearn_status_str, "LF ON %s",
                           pattern->name);
      },
      true);
}
...
bool tpms_view_receiver_input(InputEvent *event, void *context) {
  ...
  if (tpms_receiver->lock == TPMSLockOn) {
    with_view_model(
        tpms_receiver->view, TPMSReceiverModel * model,
        { model->bar_show = TPMSReceiverBarShowToUnlockPress; }, true);
    ...
    if (tpms_receiver->lock_count >= UNLOCK_CNT) {
      ...
      with_view_model(
          tpms_receiver->view, TPMSReceiverModel * model,
          { model->bar_show = TPMSReceiverBarShowUnlock; }, true);
      tpms_receiver->lock = TPMSLockOff;
  ...
  if (event->key == InputKeyBack && event->type == InputTypeShort) {
    tpms_receiver->callback(TPMSCustomEventViewReceiverBack,
                            tpms_receiver->context);
    ...
    with_view_model(
        tpms_receiver->view, TPMSReceiverModel * model,
        {
          if (model->history_item && model->idx != model->history_item - 1)
            model->idx++;
        },
        true);
  } else if (event->key == InputKeyLeft && event->type == InputTypeShort) {
    tpms_receiver->callback(TPMSCustomEventViewReceiverConfig,
                            tpms_receiver->context);
    ...
    with_view_model(
        ...
        {
          if (model->history_item != 0) {
            tpms_receiver->callback(TPMSCustomEventViewReceiverOK,
                                    tpms_receiver->context);
          }
        },
        false);
...
void tpms_view_receiver_enter(void *context) { furi_assert(context); }
  ...
  with_view_model(
      tpms_receiver->view, TPMSReceiverModel * model,
      {
        ...
        model->relearn_active = false;
                for
                  M_EACH(item_menu, model->history->data,
                         TPMSReceiverMenuItemArray_t) {
...
TPMSReceiver *tpms_view_receiver_alloc() {
  ...
  tpms_receiver->relearn_active = false;
  tpms_receiver->relearn_burst_phase_on = false;
  view_allocate_model(tpms_receiver->view, ViewModelTypeLocking,
                      sizeof(TPMSReceiverModel));
  view_set_context(tpms_receiver->view, tpms_receiver);
  view_set_draw_callback(tpms_receiver->view,
                         (ViewDrawCallback)tpms_view_receiver_draw);
  ...
  with_view_model(
      ...
      true);
  tpms_receiver->lock_timer = furi_timer_alloc(
      tpms_view_receiver_lock_timer_callback, FuriTimerTypeOnce, tpms_receiver);
  tpms_receiver->relearn_runtime_timer =
      furi_timer_alloc(tpms_view_receiver_relearn_timer_callback,
                       FuriTimerTypeOnce, tpms_receiver);
  tpms_receiver->relearn_burst_timer = furi_timer_alloc(
      tpms_relearn_burst_timer_callback, FuriTimerTypeOnce, tpms_receiver);
...
uint16_t tpms_view_receiver_get_idx_menu(TPMSReceiver *tpms_receiver) {
  ...
  uint32_t idx = 0;
  with_view_model(
      tpms_receiver->view, TPMSReceiverModel * model, { idx = model->idx; },
      false);
  return idx;
}

void tpms_view_receiver_set_idx_menu(TPMSReceiver *tpms_receiver,
                                     uint16_t idx) {
  furi_assert(tpms_receiver);
  with_view_model(
      tpms_receiver->view, TPMSReceiverModel * model,
...
void tpms_view_receiver_set_relearn_config(TPMSReceiver *tpms_receiver,
                                           TPMSRelearn relearn,
