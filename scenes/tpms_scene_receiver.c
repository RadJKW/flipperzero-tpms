...
static const NotificationSequence subghz_sequence_rx = {
    ...
    &message_vibro_on,  &message_note_c6,
    &message_delay_50,  &message_sound_off,
    &message_vibro_off,
...
static void tpms_scene_receiver_update_statusbar(void *context) {
  ...
  if (!tpms_history_get_text_space_left(app->txrx->history, history_stat_str)) {
    ...
    tpms_view_receiver_add_data_statusbar(
        app->tpms_receiver, furi_string_get_cstr(frequency_str),
...
static void tpms_scene_receiver_add_to_history_callback(
    SubGhzReceiver *receiver, SubGhzProtocolDecoderBase *decoder_base,
    void *context) {
  furi_assert(context);
  TPMSApp *app = context;
  FuriString *str_buff;
  ...
  if (tpms_history_add_to_history(app->txrx->history, decoder_base,
                                  app->txrx->preset) ==
      TPMSHistoryStateAddKeyNewDada) {
    furi_string_reset(str_buff);

    tpms_history_get_text_item_menu(app->txrx->history, str_buff,
                                    tpms_history_get_item(app->txrx->history) -
                                        1);
    tpms_view_receiver_add_item_to_menu(
        app->tpms_receiver, furi_string_get_cstr(str_buff),
...
void tpms_scene_receiver_on_enter(void *context) {
  ...
  if (app->txrx->rx_key_state == TPMSRxKeyStateIDLE) {
    tpms_preset_init(app, "AM650",
                     subghz_setting_get_default_frequency(app->setting), NULL,
                     0);
  ...
  tpms_view_receiver_set_relearn_config(
      app->tpms_receiver, app->relearn, app->relearn_pattern,
      app->relearn_runtime, app->relearn_duty);
  ...
  tpms_view_receiver_set_callback(app->tpms_receiver,
                                  tpms_scene_receiver_callback, app);
  ...
  tpms_view_receiver_set_idx_menu(app->tpms_receiver,
  ...
  if (event.type == SceneManagerEventTypeCustom) {
    ...
    case TPMSCustomEventViewReceiverBack:
      ...
      tpms_preset_init(app, "AM650",
                       subghz_setting_get_default_frequency(app->setting), NULL,
                       0);
      if (scene_manager_has_previous_scene(app->scene_manager,
                                           TPMSSceneStart)) {
    ...
    case TPMSCustomEventViewReceiverOK:
      app->txrx->idx_menu_chosen =
          tpms_view_receiver_get_idx_menu(app->tpms_receiver);
...
void tpms_scene_receiver_on_exit(void *context) { UNUSED(context); }
